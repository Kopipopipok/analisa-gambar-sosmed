...
def additional_metrics(img_pil): analysis = {} img = np.array(img_pil) img_resized = cv2.resize(img, (300, 300)) gray = cv2.cvtColor(img_resized, cv2.COLOR_RGB2GRAY)
...
def extract_text_ocr_space(img_pil): buffered = cv2.imencode(".jpg", np.array(img_pil))[1].tobytes() response = requests.post( "https://api.ocr.space/parse/image", data={"apikey": "K85290270188957"}, files={"filename": buffered}, ) try: text = response.json()["ParsedResults"][0]["ParsedText"] return text except: return ""

def extract_text_insights(text): lines = text.split("\n") insights = [] has_cta = False for line in lines: line = line.strip() if not line: continue if len(line) < 5: continue if any(word in line.lower() for word in ["diskon", "gratis", "sekarang", "%", "promo"]): insights.append(f"✅ Call-to-Action ditemukan: '{line}'") has_cta = True else: insights.append(f"ℹ️ Teks umum: '{line}'") return insights, has_cta

def predict_engagement(color_spectrum, luminance, object_count, symmetry, has_cta): score = 0 if has_cta: score += 1 if 2 <= color_spectrum <= 5: score += 1 if 100 < luminance < 220: score += 1 if 1 <= object_count <= 5: score += 1 if symmetry > 0.6: score += 1
...
def generate_recommendations(color_spectrum, luminance, object_count, symmetry, has_cta): recs = [] if not has_cta: recs.append("💡 Tambahkan teks call-to-action seperti 'Diskon', 'Beli sekarang', atau '% gratis'") if not (2 <= color_spectrum <= 5): recs.append("🎨 Gunakan kombinasi warna yang tidak terlalu banyak agar lebih fokus") if not (100 < luminance < 220): recs.append("💡 Atur kecerahan gambar agar tidak terlalu gelap atau terlalu terang") if not (1 <= object_count <= 5): recs.append("📷 Kurangi jumlah elemen visual agar tidak terlalu ramai") if symmetry <= 0.6: recs.append("📐 Perbaiki komposisi visual agar lebih simetris dan enak dilihat") return recs

if uploaded_file: image = Image.open(uploaded_file).convert("RGB") st.image(image, caption="Gambar yang Diunggah", use_container_width=True)
